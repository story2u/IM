...
import { demoOpportunities, featuredDemoOpportunity } from '@/lib/demo/opportunity-demo-data'
...
function PreviewCard({ index }: { index: number }) {
  const opportunity = demoOpportunities[index]
  return (
    <div className="rounded-md border bg-card p-3 shadow-sm" data-testid={`landing-opportunity-${index}`}>
      <div className="flex items-start gap-2.5">
        <span className="grid size-8 shrink-0 place-items-center rounded-md bg-muted"><PlatformIcon platform={opportunity.platform} className="size-4" /></span>
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5"><strong className="truncate text-xs">{opportunity.contactName}</strong><Badge variant="outline" className="h-4 px-1 text-[9px]">{opportunity.priority === 'urgent' ? '紧急' : opportunity.priority === 'high' ? '高' : '普通'}</Badge></div>
          <p className="mt-1 line-clamp-2 text-[11px] leading-4 text-muted-foreground">{opportunity.summary}</p>
        </div>
        <span className="text-sm font-semibold text-primary">{Math.round(opportunity.confidenceScore * 100)}%</span>
      </div>
      <div className="mt-2 flex items-center justify-between text-[9px] text-muted-foreground"><span>{opportunity.groupName ?? '私聊'} · {opportunity.platform === 'telegram' ? 'Telegram' : '企业微信'}</span><span>可信度 {opportunity.trustScore}</span></div>
    </div>
  )
}
...
export function ProductHome() {
  ...
  const experienceHref = demoEnabled ? '/demo' : '/login'
    ...
    <div className="min-h-svh bg-background text-foreground" data-testid="product-home">
      <header className="sticky top-0 z-40 border-b bg-background/92 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 lg:px-8">
          ...
          <div className="flex items-center gap-2"><Button nativeButton={false} render={<Link href="/login" />} variant="ghost" size="sm">登录</Button><Button nativeButton={false} render={<Link href={experienceHref} />} size="sm" data-testid="start-experience">开始体验</Button></div>
        </div>
      </header>
      ...
      <main>
        <section className="border-b" data-testid="hero">
          <div className="mx-auto grid min-h-[calc(100svh-4rem)] max-w-7xl content-center gap-10 px-4 py-14 lg:grid-cols-[0.88fr_1.12fr] lg:px-8 lg:py-16">
            <div className="flex flex-col justify-center">
              ...
              <div className="mt-8 flex flex-wrap gap-3"><Button size="lg" nativeButton={false} render={<Link href={experienceHref} />} className="gap-2">开始体验<ArrowRight className="size-4" /></Button><Button size="lg" variant="outline" nativeButton={false} render={<a href="#demo" />} className="gap-2 bg-transparent">查看演示</Button></div>
              ...
              <div className="w-full overflow-hidden rounded-md border bg-muted/35 shadow-xl">
                ...
                <div className="grid min-h-[500px] grid-cols-[116px_1fr] sm:grid-cols-[150px_1fr]">
                  ...
                  <div className="min-w-0 p-3 sm:p-4">
                    ...
                    <div className="mb-3 rounded-md border border-warning/40 bg-warning/10 p-2.5"><p className="flex items-center gap-1.5 text-[10px] font-semibold"><Sparkles className="size-3 text-warning" />Pi Agent 发现 2 条重大商机</p><p className="mt-1 text-[9px] text-muted-foreground">优先核对链接结论与后续行动建议</p></div>
                    <div className="grid gap-2 xl:grid-cols-2"><PreviewCard index={0} /><PreviewCard index={1} /><PreviewCard index={3} /><PreviewCard index={4} /></div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <section id="demo" className="border-b py-20" data-testid="agent-analysis"><div className="mx-auto grid max-w-7xl gap-12 px-4 lg:grid-cols-2 lg:px-8"><div><p className="text-sm font-semibold text-primary">Pi Agent 分析链</p><h2 className="mt-2 text-3xl font-semibold">把一句需求整理成可执行的审核材料</h2><blockquote className="mt-7 border-l-2 border-primary pl-5 text-xl leading-8">“{featuredDemoOpportunity.summary.split('预算')[0]}”</blockquote><p className="mt-6 text-sm leading-6 text-muted-foreground">Pi Agent 提供分析和建议，外部动作仍需人工批准。Telegram 凭据不会发送给模型。</p></div><div className="rounded-md border bg-card p-5 shadow-sm"><div className="grid grid-cols-2 gap-3"><Metric label="商机相关度" value="96%" /><Metric label="可信度" value="91 / 100" /></div><dl className="mt-5 space-y-4 text-sm"><AnalysisRow label="命中关键词" value="采购 50 套 · 安排演示 · 本月评估" /><AnalysisRow label="联系方式" value="procurement@example.com · @demo_buyer" /><AnalysisRow label="风险核验" value="无外部链接，消息来源已连接" /><AnalysisRow label="建议动作" value="确认演示时间和设备规格，需要人工批准" /><AnalysisRow label="回复草稿" value="可以安排演示，请问下周二或周三哪天方便？" /></dl></div></div></section>
      ...
      <footer className="border-t py-8"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 text-xs text-muted-foreground lg:px-8"><Brand /><span>在线 Demo 需要登录 · Android APK 即将提供 · iOS TestFlight 尚未开放</span></div></footer>
    </div>
  )
}

function Metric({ label, value }: { label: string; value: string }) { return <div className="rounded-md bg-muted p-3"><span className="text-xs text-muted-foreground">{label}</span><strong className="mt-1 block text-xl font-semibold text-primary">{value}</strong></div> }
function AnalysisRow({ label, value }: { label: string; value: string }) { return <div className="grid gap-1 border-t pt-3 sm:grid-cols-[100px_1fr]"><dt className="text-muted-foreground">{label}</dt><dd>{value}</dd></div> }
