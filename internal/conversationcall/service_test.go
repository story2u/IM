...
func TestCallCreatesVoiceCallTaskFromConversationSnapshot(t *testing.T) {
	...
	if creator.request.Source != "system" || creator.request.Target.AgentID != "agent-1" || creator.request.Target.DeviceID != "device-1" || creator.request.TaskType != "voice_call" {
...
func TestCallCreatesVideoTaskAndGeneratedReservation(t *testing.T) {
	...
	if creator.request.TaskType != "video_call" || creator.request.Payload["receiver"] != "external-1" || payload["reservation_id"] != "reservation-01" {
...
func TestHangupCreatesHangupTask(t *testing.T) {
	...
	if payload["success"] != true || creator.request.TaskType != "hangup_call" || creator.request.Payload["receiver"] != "Alice" {
