// Package workbenchaiconfig reads system_settings values for the admin AI
// config candidate. It is intentionally read-only and leaves AI config writes,
// account AI synchronization, audit logs, and broadcasts in Python.
...
// Queryer is the database/sql shape needed by the AI config repository.
type Queryer interface {
	QueryContext(ctx context.Context, query string, args ...any) (RowsScanner, error)
	ExecContext(ctx context.Context, query string, args ...any) (sql.Result, error)
}

// Repository reads AI config values from system_settings.
...
// GetAIConfigValue returns one raw system_settings value or an empty string.
func (repository *Repository) GetAIConfigValue(ctx context.Context, key string) (string, error) {
	if repository.DB == nil {
		return "", fmt.Errorf("workbench ai config database is not configured")
	}
...
// SetAIConfigValue upserts one system_settings value.
func (repository *Repository) SetAIConfigValue(ctx context.Context, key string, value string) error {
	if repository.DB == nil {
		return fmt.Errorf("workbench ai config database is not configured")
	}
	normalizedKey := strings.TrimSpace(key)
