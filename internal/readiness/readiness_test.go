...
func TestEvaluateReadyProfile(t *testing.T) {
	...
	report := Evaluate(profile, Inputs{
		...
		Env: map[string]string{
			...
			"CLOUD_DB_DSN":                           "postgres://db",
			"SESSION_JWT_SECRET":                     "secret",
			"GO_SEND_CONNECTOR_BASE_URL":             "https://send-connector.local",
...
func TestEvaluateDeviceOpsProfile(t *testing.T) {
	...
	report := Evaluate(profile, Inputs{
		...
		Env: map[string]string{
			...
			"RPA_CALL_AUDIO_BRIDGE_STATUS_FILE":          "/app/data/bridge-status.json",
			"RPA_CALL_AUDIO_BRIDGE_TARGETS_FILE":         "/app/data/bridge-targets.json",
			"RPA_CALL_AUDIO_BRIDGE_HOST_DATA_ROOT":       "/app/data",
