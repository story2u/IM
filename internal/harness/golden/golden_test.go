...
func TestCompareMatchesCanonicalJSON(t *testing.T) {
	baseline := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"ok":true,"trace_id":"baseline","data":{"items":[{"name":"a","updated_at":"old"}]}}`))
	}))
...
func TestCompareReportsStatusAndBodyDrift(t *testing.T) {
	...
	result, err := Compare(context.Background(), nil, Endpoint{Name: "baseline", BaseURL: baseline.URL}, Endpoint{Name: "go", BaseURL: goTarget.URL}, Case{Name: "probe"}, Options{})
	if err != nil {
		t.Fatalf("Compare returned error: %v", err)
	}
	if result.Match {
	...
	if !strings.Contains(lines, "status: baseline=200 go=503") {
...
func TestCompareCanSkipBodyComparison(t *testing.T) {
	baseline := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte("baseline metrics"))
	}))
	defer baseline.Close()
	...
	result, err := Compare(context.Background(), nil, Endpoint{Name: "baseline", BaseURL: baseline.URL}, Endpoint{Name: "go", BaseURL: goTarget.URL}, Case{
		Name:            "metrics",
		Path:            "/metrics",
