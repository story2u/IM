...
- 工作机会模式从授权群聊中区分招聘、求职自荐、广告和诈骗，提取有原文证据的职位并按用户职业偏好做确定性匹配。
...
工作机会垂直链路在同一消息与 Agent Runtime 上运行：来源画像 → 噪音预筛 → 招聘字段证据校验 →
重复来源聚合 → 求职档案匹配。系统不会自动投递简历或联系招聘方，也不使用年龄、性别等受保护属性
进行推荐。
...
分发与真实支付 Sandbox E2E 仍为 Beta 或待外部验证。工作机会三端页面、API、确定性匹配和虚构评估
夹具已实现，真实群金标识别效果尚未验证。权威状态见[功能地图](docs/product/feature-map.md)。
...
- 使用经授权、脱敏的招聘群金标样本评估工作机会分类、提取和去重阈值。
