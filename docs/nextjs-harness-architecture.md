...
- 客服工作台：会话列表、消息面板、发送入口、实时状态、客户资料。
- 管理台：账号、分配、配置、内容、诊断、任务和审计。
- 运维入口：版本、健康状态、错误上报、发布验证 smoke。
...
1. 先约束页面边界，再实现交互行为。
   - 先确认路由、导航、登录态、版本和基本入口可用，再逐步引入发送、搜索、实时回放等交互。
...
- 命令：`cd go && node scripts/next-routes.mjs web/app --check --markdown`
   - 输出：`go/tmp/release-gate/web-routes.json`、`go/tmp/release-gate/web-routes.md`
- 覆盖：
...
```bash
cd go
RUN_WEB_E2E=1 SKIP_NPM_CI=1 bash scripts/release_gate.sh
```
