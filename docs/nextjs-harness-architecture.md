...
> 本文档定义 Go 实现阶段中 Next.js 前端的可验证链路。
> 对齐“先导航、先清单、先验证”的工程理念，并把前端证据沉淀为可回放产物。

## 1. 定位与边界

前端实现不能“推断业务事实”，只能验证并承接 Go API 与兼容基线的契约。
...
2. **前端只验证“展示/交互契约”**
   业务裁剪、分页口径、状态归类必须在 Go API 或兼容基线 API 契约里验证。
...
5. **高频变更留白口径**
   `node --test` 与 Playwright 的“低成本断言”先于 baseline/Go 全量联调。
