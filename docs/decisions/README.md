...
- [ADR-0006：移动端作为现有 API 的瘦客户端](0006-mobile-app-thin-client.md)（superseded by ADR-0009）
...
- [ADR-0009：移动端迁移到 React Native + Expo 单代码库](0009-react-native-expo-mobile-client.md)（accepted）
- [ADR-0010：端上 pi Agent 优先，服务端网关与 runner 兜底](0010-device-agent-with-hosted-fallback.md)（accepted）
- [ADR-0011：先用服务端变更日志同步，不立即切换事件溯源权威](0011-server-change-feed-before-event-sourcing.md)（accepted）
- [ADR-0009：策略门控的 AI 自动回复](0009-policy-gated-ai-auto-reply.md)（accepted）
- [ADR-0010：默认启用 AI 回复流水线](0010-enable-ai-reply-pipeline-by-default.md)（accepted）
- [ADR-0011：职位发现证据与确定性匹配](0011-job-discovery-evidence-and-deterministic-matching.md)（accepted）
