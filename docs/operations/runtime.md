...
| Password reset | `PASSWORD_RESET_ENABLED`、`PASSWORD_RESET_*`、`RESEND_API_KEY`、`SMTP_*` | 默认开启；生产 Workflow 将 Resend API Key Secret 映射为固定 SMTP 配置，未配置时 fail closed，详见密码重置 Runbook |
