...
| 企业微信 | `WECOM_CONNECTION_LIMIT`、`WECOM_WEBHOOK_TOLERANCE_SECONDS`、`WECOM_WEBHOOK_MAX_BODY_BYTES` | 新用户的 CorpID/AgentID/Secret/Token/AES Key 由 `/settings/wecom` 录入并加密存库；旧 `WECOM_*` 仅保留全局回调兼容 |
