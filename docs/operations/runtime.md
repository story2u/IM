...
| Password reset | `PASSWORD_RESET_ENABLED`、`PASSWORD_RESET_*`、`SMTP_HOST/PORT/USERNAME/PASSWORD/FROM_*`、`SMTP_STARTTLS/USE_TLS` | 默认关闭；SMTP 密码只进 Secret；开启前需验证发件域和送达，详见密码重置 Runbook |
