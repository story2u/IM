...
| Mira 今日简报与消息管家 | H5 `/`、`/messages`、`/mira`；RN `/(tabs)/home`、`/(tabs)/dashboard`、`/(tabs)/agent` | H5 读取服务端商机列表并投影 Mira 统计；RN 本地 SQLite Briefing/AttentionSnapshot；interactive schema v5 | 部分实现 | H5 登录后默认进入 OpenMIRA 三标签体验，公开页、登录页、桌面侧栏和移动底栏统一为 OpenMIRA/Mira 心智；读取失败会保留明确重试态，不再静默显示 0。RN 新体验默认在 dev 开启，CI production 构建显式开启 `EXPO_PUBLIC_MIRA_CONCIERGE_UI_ENABLED=true`，底部导航为“今天 / 消息 / Mira”，设置降为首页头像入口，并可用 `EXPO_PUBLIC_MIRA_CONCIERGE_UI_ENABLED=false` 回到经典首页/看板/Account Tab。Today 读取本地 AttentionSnapshot 与 Briefing 列表，展示 Mira 状态头、今日只需看数量、一句话、三时段简报和 Reduce Motion 友好的 MiraOrb；Messages 默认显示智能分类，原看板列表通过分类进入。简报可按本地日程到点生成，同日同一时段去重，L2 summarizer 失败降级为本地结构化简报。真机动效、截图 golden、通知实际投递仍待验收 |
...
| 商机看板（服务端筛选/排序/分页） | RN 消息 Tab 经典列表、H5 `/messages` 深层列表 | `GET /opportunities/dashboard`、H5 `GET /opportunities` | 部分实现 | 后端聚合端点已实现：status/platform/source/时间范围/trust/sop/keyword 多维筛选 + 4 种排序 + total/pendingCount/attentionItems/keywordOptions，全部 SQL 完成并按 owner 隔离，有 Postgres 门控筛选矩阵测试；RN dev 包已接入共享严格 decoder、全量服务端筛选/排序/分页、重大提示、Abort/竞态保护及 loading/error/empty/retry。2026-07-20 起 RN 默认“消息”入口先展示 Mira 智能分类，旧看板列表通过 `view=list` 和 feature flag 继续可达。P3 S4 已实现同语义 SQLite 只读查询，但生产 capability 默认关闭且真机飞行模式待 S7。H5 `/messages` 改为 Mira 智能分类和深层列表，筛选数据来自共享严格 API 边界的服务端列表投影；旧 Web 筛选面板不再是登录后 `/` 默认体验。SwiftUI/Compose 双 Tab 已接入但未在 CI 外做真机 E2E。iOS 在生产后端滚动升级期间会把默认/status/platform 查询降级到旧 `/opportunities`，并明确停用无法等价支持的高级筛选 |
