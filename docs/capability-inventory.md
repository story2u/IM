...
| 以具体通道命名的 HTTP routes | `/api/v1/connectors/*` 主入口，`/wework/*` 兼容入口，notify routes, device SDK open/stop routes | 继续移到 connector/provider admin namespace，保留旧 route 作为 adapter-only 兼容入口并补下线条件 |
